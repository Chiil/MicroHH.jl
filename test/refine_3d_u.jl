    ...
    elseif n_hi ÷ n_lo == 3
        @tturbo for k in 0:size(lo, 3)-3, j in 0:size(lo, 2)-3, i in 0:size(lo, 1)-3
            ...
            for kk in 0:2, jj in 0:2
                ...
                hi_tmp[i_hi+2, j_hi+jj, k_hi+kk] = lo[i_lo, j_lo, k_lo]
            end
        end
...
a_hi_int = a_hi_gc[2:end, 2:end-1, 2:end-1]
...
a_hi_int_ref = a_hi_gc[2:end, 2:end-1, 2:end-1]
...
x_hi_p = dx_hi/2:dx_hi:1 |> collect
x_lo_p = dx_lo/2:dx_lo:1 |> collect
pushfirst!(x_hi_p, 0)
pushfirst!(x_lo_p, 0)
...
figure(figsize=(10, 5))
subplot(121)
pcolormesh(x_lo_p, xh_lo, a_lo[:, :, 1]', vmin=0, vmax=1)
xlim(0, 1)
ylim(0, 1)
subplot(122)
pcolormesh(x_hi_p, xh_hi, a_hi_int[:, :, 1]', vmin=0, vmax=1)
xlim(0, 1)
ylim(0, 1)
tight_layout()
display(gcf())
